...
export async function executorAppServerFixture(t, { overrides = {}, validateTarget = async () => true, dispatcherOptions = {}, validateRecovery = null } = {}) {
  ...
  const handlers = {
    ...
    "thread/read": () => ({ thread: { id: "owned-task", name: state.name, cwd: member.target.cwd, turns: state.turns } }),
  ...
  const effects = new ExecutorAppServerEffectsV1({ session, validateTarget, validateRecovery });
  await session.open();
