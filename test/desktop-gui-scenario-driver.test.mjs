...
test("boundary assertion values are normalized to booleans", async () => {
  const { boundary, driver } = await harness();
  boundary.taskState = async () => ({ matched: true });
  const result = await driver.runScenario(desktopGuiScenario());
  assert.equal(result.outcome, "passed");
  assert.equal(result.assertions.find(({ assertionId }) => assertionId === "state").passed, true);
});
