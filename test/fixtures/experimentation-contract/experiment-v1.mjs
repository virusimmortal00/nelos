...
export function buildExperimentV1(overrides = {}) {
  const value = {
    ...
    corpus: { releaseId: `corpus:${"4".repeat(64)}`, digest: D("4") },
    design: {
    ...
    runtimeMatrix: [{
      runtimeLockId: `runtime:${"5".repeat(64)}`,
      digest: D("5"),
...
export const invalidExperimentFixturesV1 = Object.freeze([
  ...
  { name: "primary metric repeated as secondary", code: "DUPLICATE_IDENTITY", path: "/metrics/secondary/0/metricId", mutate: (v) => { v.metrics.secondary[0].metricId = v.metrics.primary.metricId; } },
  { name: "named corpus reference", code: "OUT_OF_BOUNDS", path: "/corpus/releaseId", mutate: (v) => { v.corpus.releaseId = "corpus:milestone-one"; } },
  { name: "named runtime reference", code: "OUT_OF_BOUNDS", path: "/runtimeMatrix/0/runtimeLockId", mutate: (v) => { v.runtimeMatrix[0].runtimeLockId = "runtime:linux"; } },
  { name: "desktop backend on Linux", code: "INVALID_FORMAT", path: "/runtimeMatrix/0/platform", mutate: (v) => { v.runtimeMatrix[0].backend = "dedicated-desktop"; } },
  { name: "OCI backend on macOS", code: "INVALID_FORMAT", path: "/runtimeMatrix/0/platform", mutate: (v) => { v.runtimeMatrix[0].platform = "macos-arm64"; } },
  { name: "candidate without an eligible runtime", code: "INVALID_FORMAT", path: "/candidates/1/candidateId", mutate: (v) => { v.runtimeMatrix[0].eligibleCandidateIds = [v.candidates[0].candidateId]; } },
  { name: "promotion rule references undeclared metric", code: "INVALID_FORMAT", path: "/decisionRules/promotion/metricId", mutate: (v) => { v.decisionRules.promotion.metricId = "missing_metric"; } },
  { name: "regression rule references undeclared metric", code: "INVALID_FORMAT", path: "/decisionRules/regression/metricId", mutate: (v) => { v.decisionRules.regression.metricId = "missing_metric"; } },
  { name: "stop rule references undeclared metric", code: "INVALID_FORMAT", path: "/decisionRules/stop/metricId", mutate: (v) => { v.decisionRules.stop.metricId = "missing_metric"; } },
