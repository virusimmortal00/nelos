...
test("wraps missing visual reports without exposing filesystem details", async () => {
  const missing = await fixture();
  await rm(missing.report.path);
  await assert.rejects(validateArchiveProjectionConvergence(missing.input), (error) => {
    assert.equal(error.code, "INVALID_VISUAL_REPORT");
    assert.doesNotMatch(error.message, /ENOENT|private|nelos-archive-convergence/u);
    return true;
  });
});
