...
| Member needs correction | If a queen-rejected or contract-malformed result has current turn provenance, follow-up capability, and attempt budget remaining, persist `correction-pending`, consume the typed same-task follow-up receipt, advance the attempt, and rejoin the later turn; otherwise surface attention without an unusable follow-up. Consumed malformed receipts remain immutable audit evidence and are never rebound to changed output. |
