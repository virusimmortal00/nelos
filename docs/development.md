...
The MCP bridge is pinned more narrowly because it performs a title mutation.
After a Codex upgrade, generate the experimental schema into a temporary
directory, compare the initialization response plus `thread/read` and
`thread/name/set`, then update the compact protocol fixture and supported
version together:
