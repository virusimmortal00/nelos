...
VERSION=$(node -p 'require(process.argv[1]).version' \
  "$HOME/.codex/plugins/cache/nelos-marketplace/nelos/0.9.0/.codex-plugin/plugin.json")
test "$VERSION" = "0.9.0"
node -e 'const fs=require("node:fs"); const p=JSON.parse(fs.readFileSync(process.argv[1]));
  ...
  "$HOME/.codex/plugins/cache/nelos-marketplace/nelos/0.9.0/distribution-provenance.json"
