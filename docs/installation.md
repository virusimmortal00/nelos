...
VERSION=$(node -p 'require(process.argv[1]).version' \
  "$HOME/.codex/plugins/cache/nelos-marketplace/nelos/0.5.1/.codex-plugin/plugin.json")
test "$VERSION" = "0.5.1"
node -e 'const fs=require("node:fs"); const p=JSON.parse(fs.readFileSync(process.argv[1]));
  ...
  "$HOME/.codex/plugins/cache/nelos-marketplace/nelos/0.5.1/distribution-provenance.json"
