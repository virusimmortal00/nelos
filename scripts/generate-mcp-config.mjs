...
export function buildMcpBootstrap() {
  return [
    ...
    "  } catch (error) {",
    '    if (error.code !== "ENOENT") fail("cannot read the plugin cache at " + cacheRoot + ": " + error.message);',
    "  }",
    "  const candidates = [];",
    ...
    '    const images = path.join(process.env.XDG_STATE_HOME || path.join(os.homedir(), ".local", "state"), "nelos", "runtime-images", version);',
    '    let retained = [];',
    '    try { retained = fs.readdirSync(images).filter((name) => /^nelos-build-[a-f0-9]{32}$/.test(name)); } catch (error) { if (error.code !== "ENOENT") throw error; }',
    '    for (const name of retained) candidates.push(path.join(images, name, "src", "mcp-server.mjs"));',
    "  }",
    "  if (candidates.length === 0) {",
  ...
  '      "; keep exactly one installed nelos plugin copy before restarting Codex");',
    ...
    "    }",
    '    const loader = await import(pathToFileURL(path.join(pluginRoot, "src", "runtime-retention.mjs")).href);',
    "    await loader.startRetainedNelosMcpServerV1({ moduleRoot: pluginRoot, serverVersion: version });",
    "  } catch (error) {",
