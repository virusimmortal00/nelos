...
export function executorMcpToolsV1(client) {
  ...
  return [
    ...
    ["launch", "Start ready work in the job or dependency plan already authorized by the service operator. Existing workers are never replayed. No target, prompt, model, or permission changes are accepted."],
    ...
    inputSchema: { type: "object", properties: method === "join" ? {
      workUnitId: { type: "string", minLength: 1, maxLength: 128, description: "Required for an attached multi-worker plan." },
      expectedAttempt: { type: "integer", minimum: 1, maximum: 3 },
      decision: { type: "string", enum: ["accepted", "rejected"] },
      decisionSummary: { type: "string", minLength: 1, maxLength: 1000 },
    } : method === "acknowledge" ? { workUnitId: { type: "string", minLength: 1, maxLength: 128 }, notificationId: { type: "string", pattern: "^completion:[a-f0-9]{64}$" },
      expectedAttempt: { type: "integer", minimum: 1, maximum: 3 } }
      : method === "retry" ? { expectedAttempt: { type: "integer", minimum: 1, maximum: 3 } }
      : method === "collect" ? { workUnitId: { type: "string", minLength: 1, maxLength: 128, description: "Required for an attached multi-worker plan." } } : {},
